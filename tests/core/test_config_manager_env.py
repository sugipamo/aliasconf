...
class TestConfigManagerEnvIntegration:
    ...
    def test_load_from_env_without_prefix(self):
        ...
        with mock.patch.dict(
            os.environ, {"DATABASE_HOST": "prod.db.com", "API_KEY": "secret123"}
        ):
            config = ConfigManager()
    ...
    def test_load_from_env_merge_strategy_replace(self):
        ...
        with mock.patch.dict(
            ...
            assert config.get("database.port", int) == 5432
            assert (
                config.get("database.name", str) == "myapp"
            )  # REPLACEでも部分的な置き換え
    ...
    def test_load_from_env_merge_strategy_override(self):
        ...
        with mock.patch.dict(
            os.environ, {"ALIASCONF_DATABASE": '{"host": "prod.db.com", "port": 5432}'}
        ):
    ...
    def test_load_from_env_with_custom_converter(self):
        ...
        with mock.patch.dict(
            ...
            assert config.get("allowed.ips.list", list) == [
                "192.168.1.1",
                "192.168.1.2",
                "192.168.1.3",
            ]
    ...
    def test_load_from_env_error_handling(self):
        ...
        with mock.patch.dict(
            ...
            assert (
                config.get("invalid.json", str) == '{"invalid": json}'
            )  # 文字列として保持
    ...
    def test_load_from_env_with_aliases(self):
        ...
        with mock.patch.dict(
            os.environ, {"ALIASCONF_DB_HOST": "prod.db.com"}  # エイリアス経由
        ):
    ...
    def test_load_from_env_method_chaining(self):
        ...
        with mock.patch.dict(os.environ, {"ALIASCONF_APP_NAME": "MyApp"}):
            config = ConfigManager()
